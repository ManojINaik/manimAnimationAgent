...
class Scene2_Helper:
    ...
    def create_root_system(self, soil):
        ...
        root_edge = soil.get_edge()
        root_line_1 = Line(start=root_edge + LEFT*0.3 + UP*0.0, end=root_edge + UP*1.5 + LEFT*0.5, stroke_color=WHITE, stroke_width=2)
        root_line_2 = Line(start=root_edge, end=root_edge + UP*1.8, stroke_color=WHITE, stroke_width=2)
        root_line_3 = Line(start=root_edge + RIGHT*0.3 + UP*0.0, end=root_edge + UP*1.5 + RIGHT*0.5, stroke_color=WHITE, stroke_width=2)
    ...
    def create_water_arrows(self, soil, root_line_2):
        ...
        water_arrows = VGroup()
        for i in range(3):
            arrow = Arrow(start=soil.get_edge() + DOWN*0.1 + LEFT*0.4 + RIGHT*(i*0.4), end=root_line_2.get_start() + DOWN*0.2 + LEFT*0.1, color=BLUE)
